...
export const DashboardSideBar = ( { session }: any ) => {
    ...
    <div className=" flex flex-col py-4 px-2 space-y-3">
        {
            privateLinks.map((single) => {
                return (
                    <Link href={single.url} key={single.id} className={`hover:bg-yellow-500/30 ${single.name == 'Dashboard' && ' bg-yellow-500' } rounded-md space-x-2 w-full py-3 px-4 flex flex-row`}>
